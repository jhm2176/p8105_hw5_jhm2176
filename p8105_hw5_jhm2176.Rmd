...
First, I will fix n=30, fix σ=5, and set μ=0.

```{r}
rnorm(n = 30, mean = 0, sd = 5) #update?
```

Next, I will generate 5000 datasets from the model x∼Normal[μ,σ] and save the mean estimate and p-value (H:μ=0 using α=0.05) for each.

The following code creates a function to simulate data from a normal distribution with the given sample size, mean, and standard deviation and returns estimates of the mean and p-value.
...
sim_mean_p = function(n = 30, mu, sigma = 5) {
  ...
  sim_data %>% 
    t.test(mu = mu, conf.level = 0.95) %>%
    broom::tidy() %>% unnest()
}
```

This code generates 5000 datasets and saves the mu hat and p-value for each in the data frame sim_df.

```{r}
sim_df = 
  rerun(100, sim_mean_p(mu = 0)) %>% #replace with n=5000
  bind_rows() %>% 
...
sim_df_2 = 
  tibble(mu = c(0:6)) %>% 
  mutate(
    output_lists = map(.x = mu, ~rerun(100, sim_mean_p(mu = .x))), #replace with 5000
...
power_df = 
  ...
  mutate(power = reject / (fail_to_reject + reject))
