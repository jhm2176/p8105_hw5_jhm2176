...
prop.test(x = 1825, n = 2827, conf.level = 0.95, correct = FALSE) %>% broom::tidy()
```
...
```{r}
city_homicides %>% 
  mutate(newcol = map2(n_unsolved_homicides, n_homicides,  ~ prop.test(.x, n = .y) %>% 
                  {tibble(estimate = .[["estimate"]],
                         CI_lower = .[["conf.int"]][[1]], 
                         CI_upper = .[["conf.int"]][[2]])})) %>%
  unnest() %>%
  select(-n_homicides, -n_unsolved_homicides)
```
