...
First, I will generate 5000 datasets from the model x ∼ Normal [ μ , σ ] and save the mean estimates and p-values ( H : μ = 0 at α = 0.05 ) for each. I will fix n = 30, σ = 5, and set μ = 0. I will name μ as "s_mu" so as not to be confused with the `t.test` function input "mu".
...
This code generates 5000 datasets given μ = 0, maps my function `sim_mean_p` on each dataset, and saves the mean estimate and p-value for each. The resulting data frame "sim_df_0" contains estimates and p-values for each of these datasets.
...
This plot shows that the sample average of μ̂across tests for which the null is rejected is only approximately equal to the true value of μ when the true μ is greater than 3. As the true population mean increases, the sample average across tests for which the null is rejected gets closer to the true population mean. This is because power increases with effect size.
